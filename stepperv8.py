...
#Doesn't give extra room, if any measurement is a little off the island could bump into things
#Rounding might compound changing position errors over time

#ensure: pico pins correct (should be accomplished)
#ensure: motor
#  direction correct (NOT YET ACCOMPLISHED!!)
#ensure: dimensions correct (should be accomplished)
...
length = 119           #height of roof in centimetres
...
# device parameters
DELAY = 0.001
spoolDiameter = 2.54        #diameter of spool in centimetres
spoolCircle = True
...
xpos_arr = [j * island_width + enclosure_width + island_side for j in range((int(width-2*enclosure_width)//island_width))] #all important x pos in between two enclosures
...
def moveTo(xpos, ypos):
    ...
    #step 0: argh
    L_length_cur = math.sqrt((xpos - eye_distance)**2 + (ypos - eye_distance)**2)
    R_length_cur = math.sqrt((width - xpos - island_mid - eye_distance)**2 + (ypos - eye_distance)**2)

    dLS = L_length_cur - L_length_prev
    dRS = R_length_cur - R_length_prev

    #step 1: figure out how much to move based on length
    L_steps = abs(dLS) / spoolCircum * 200  # 200 full steps per revolution
    R_steps = abs(dRS) / spoolCircum * 200  # 200 full steps per revolution
    '''L_angle = 360 * dLS / spoolCircum
    L_steps = round(abs(1/1.8 * L_angle))
    R_angle = 360 * dRS / spoolCircum
    R_steps = round(abs(1/1.8 * R_angle))''' #just tried to make this block better
...
L_length_prev = math.sqrt((xhome - eye_distance)**2 + (ymid - eye_distance)**2)  #length of L wire at home base 
R_length_prev = math.sqrt((width - xhome - island_mid - eye_distance)**2 + (ymid - eye_distance)**2)  #length of R wire at home base
