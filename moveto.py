#MOVE TO TEST FROM ENCODING START AND END POSITION
...
starting_island_x = 20#dist to top left part of island at start movement
ending_island_x = 30 #dist to top left corner of island at end movement

starting_island_y = 30 #dist from spool itself in the downwards y-direction
ending_island_y = 40


LL_start = math.sqrt((starting_island_x + island_side)**2 + (island_y_pos + island_top)**2)
RL_start = math.sqrt((enclosure_dist - starting_island_x - island_width + island_side)**2 + (starting_island_y + island_top)**2)

LL_end = math.sqrt((ending_island_x + island_side)**2 + (island_y_pos + island_top)**2)
RL_end = math.sqrt((enclosure_dist - ending_island_x - island_width + island_side)**2 + (ending_island_y + island_top)**2)
